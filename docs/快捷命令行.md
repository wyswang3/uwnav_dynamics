...
conda activate uwnav_train_cpu
...
PYTHONPATH=src python -m uwnav_dynamics.cli.train \
  -y configs/train/pooltest02_s1_lstm_v0.yaml --device cuda
...
PYTHONPATH=src python -m uwnav_dynamics.cli.eval \
  -y configs/train/pooltest02_s1_lstm_v0.yaml --split test --plots --plot_fmt png

评估主产物目录：
run.out_dir/run.variant/eval_test/

绘图主产物目录：
run.out_dir/run.variant/eval_test/plots/

观察GPU占用：
nvidia-smi --query-gpu=timestamp,name,utilization.gpu,memory.used,memory.total,power.draw,temperature.gpu --format=csv -l 1
