...
> 历史说明：本文档记录的是旧 `Vel_state / B4+U1` 阶段的离线选型理由，
> 不代表当前最终状态转移求解器。2026-05-02 replay-only 验证后的默认方案是
...
本文不是论文正文，而是基于旧阶段 `out/` 目录训练产物，对“为什么当时主线最终方案曾选择 B4+U1”给出的历史技术决策说明。

## 结论先行

旧阶段证据**曾支持**把 `B4+U1` 作为当时主线最终训练方案，但这个结论是**有前提的**：
...
> 在旧阶段实验体系下，`B4+U1` 以更好的 `MAE / masked MAE / final-step / rollout growth / tail error / bias` 离线控制前筛查表现，构成了当时更合适的主线方案；但它并未在 `dense RMSE` 上全面压倒 `U1`。
...
### 6.1 旧阶段结果是否支持将 B4+U1 作为当时最终训练方案
...
> 基于 `round2 -> round3 -> round4 -> round5` 的连续筛选结果，旧阶段最合理的主线训练方案是 `B4+U1`。
...
`B4+U1` 的核心优势不是“平均 RMSE 最低”，而是“在旧阶段控制前更关键的 masked / tail / growth / bias 指标上更像一个可进入下一阶段验证的模型”。这就是当时把它作为主线方案的主要理由。
