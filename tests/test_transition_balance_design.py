...
def test_transition_balance_state_mse_weight_increases_loss_for_state_error() -> None:
    x = torch.zeros(2, 5, 25)
    y = torch.zeros(2, 4, 9)
    target_mask = torch.ones(2, 4, 9, dtype=torch.bool)
    dY = torch.zeros(2, 4, 9)
...
def test_transition_monitor_supports_single_step_horizon() -> None:
    x = torch.zeros(2, 5, 25)
    y = torch.zeros(2, 1, 9)
    target_mask = torch.ones(2, 1, 9, dtype=torch.bool)
    dY = torch.zeros(2, 1, 9)
    dY[:, :, 6] = 0.5
    logvar = torch.zeros_like(dY)
    ...
    score = monitor_fn(_DummyModel(dY=dY, logvar=logvar), x, y, target_mask)
