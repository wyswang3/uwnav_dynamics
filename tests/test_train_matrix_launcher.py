...
def test_prepare_matrix_runs_resolves_run_out_dir_relative_to_matrix_config(tmp_path: Path) -> None:
    shared_root = tmp_path / "nested" / "shared"
    base_train_yaml = shared_root / "base.yaml"
    _write_yaml(
        base_train_yaml,
        {
            "run": {
            ...
            "model": {
                ...
                "use_thruster_as_replacement": True,
                "use_hydro_feat": True,
                "blocks": {"thruster_lag": {"enabled": False}, "hydro_ssm": {"enabled": False}, "damping": {"enabled": False}, "uncertainty": {"enabled": False}},
            },
            "rollout": {"y0_source": "x_last_state", "mode": "delta_cumsum"},
            "loss": {"type": "nll_diag", "logvar_clip": [-10.0, 6.0]},
            "optim": {"name": "adamw", "lr": 1e-3, "weight_decay": 1e-4, "grad_clip": 1.0},
    ...
    matrix_yaml = tmp_path / "nested" / "configs" / "launch" / "matrix_rel.yaml"
    _write_yaml(
        matrix_yaml,
        {
            "base_train_yaml": "../../shared/base.yaml",
            "launcher": {
                "work_dir": "../../artifacts/matrix_out",
                "gpus": [0],
                "max_parallel": 1,
                "run_eval": False,
                "eval_split": "test",
    ...
    expected_run_dir = tmp_path / "nested" / "artifacts" / "matrix_out" / "runs" / "b0_seed0"
    assert prepared[0].run_dir == expected_run_dir

    generated = yaml.safe_load(prepared[0].yaml_path.read_text(encoding="utf-8"))
    resolved_out_dir = (prepared[0].yaml_path.parent / generated["run"]["out_dir"]).resolve()
    assert resolved_out_dir == expected_run_dir.parent
